...
def get_routes_blueprint(app):
    """Build the enhanced API blueprint (register with url_prefix='/v2/api')."""
    ...
    # Registered on app with url_prefix="/v2/api" so paths are /v2/api/health, etc.
    ...
    def health_check():
        ...
        try:
            ...
            performance_logger.log_request_performance(
                '/v2/api/health', 'GET', processing_time, status_code
            )
    ...
    def get_algorithms():
        ...
        try:
            algorithms_info = {
                # xlm omitted: not wired in AlgorithmManager (see validators.valid_algorithms).
                ...
                'algorithm_categories': {
                    'deep_learning': {
                        'algorithms': ['bert', 'distilbert', 'sbert'],
    ...
    def validate_files():
        ...
        try:
            ...
            processing_time = time.time() - start_time
            performance_logger.log_request_performance(
                '/v2/api/validate-files', 'POST', processing_time, 200, len(files)
            )
    ...
    def process_resumes():
        ...
        try:
            ...
            processing_time = time.time() - start_time
            performance_logger.log_request_performance(
                '/v2/api/process-resumes', 'POST', processing_time, 200,
            ...
            processing_time = time.time() - start_time
            performance_logger.log_request_performance(
                '/v2/api/process-resumes', 'POST', processing_time, 500
            )
    ...
    logger.info("API blueprint built successfully")
    return api


def create_routes(app) -> None:
    """Backward-compatible registration at /api (same paths as legacy standalone usage)."""
    app.register_blueprint(get_routes_blueprint(app))
