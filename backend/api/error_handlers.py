...
def register_error_handlers(app):
    ...
    def internal_server_error(error):
        ...
        error_id = f"error_{int(datetime.now(timezone.utc).timestamp())}_{hash(str(error)) % 10000}"
    ...
    def handle_unexpected_error(error):
        ...
        error_id = f"unexpected_{int(datetime.now(timezone.utc).timestamp())}_{hash(str(error)) % 10000}"
