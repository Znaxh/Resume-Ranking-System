...
class DatasetManager:
    ...
    def _load_persisted_extractors_if_needed(self) -> None:
        """Load TF-IDF vectorizer and feature scaler saved after training."""
        feat_path = os.path.join(
            self.models_root, "feature_extractors", "tfidf_vectorizer.joblib"
        )
        scaler_path = os.path.join(self.models_root, "scalers", "feature_scaler.joblib")
        if getattr(self, "tfidf_vectorizer", None) is None and os.path.exists(feat_path):
            self.tfidf_vectorizer = joblib.load(feat_path)
        if getattr(self, "scaler", None) is None and os.path.exists(scaler_path):
    ...
    def extract_features(self, df: pd.DataFrame, fit_transform: bool = True) -> np.ndarray:
        ...
        if not fit_transform:
            self._load_persisted_extractors_if_needed()
        ...
        if fit_transform:
            ...
            if getattr(self, "scaler", None) is None:
                raise RuntimeError(
                    "Feature scaler has not been fitted yet. Run "
                    "/api/academic/train-models first (saves data/models/scalers/feature_scaler.joblib) "
                    "or call extract_features(..., fit_transform=True) before transform-only use."
                )
    ...
    def _extract_tfidf_features(self, df: pd.DataFrame, fit_transform: bool) -> np.ndarray:
        ...
        if fit_transform:
            ...
            if getattr(self, "tfidf_vectorizer", None) is None:
                raise RuntimeError(
                    "TF-IDF vectorizer has not been fitted yet. Run "
                    "/api/academic/train-models first (saves "
                    "data/models/feature_extractors/tfidf_vectorizer.joblib) or call "
                    "extract_features(..., fit_transform=True) before transform-only use."
                )
