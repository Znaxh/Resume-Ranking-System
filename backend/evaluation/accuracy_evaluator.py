...
class AccuracyEvaluator:
    ...
    def evaluate_algorithm_predictions(
        self,
        algorithm_name: str,
        ...
        """
        Thin wrapper: normalize scores to 0–1, then reuse the same metric helpers as evaluate_algorithm.
        Does not write evaluation reports to disk (avoids side effects on API calls).
        """
        y_true = [float(x) for x in ground_truth_scores]
        y_pred = [float(x) for x in predicted_scores]

        if len(y_true) != len(y_pred):
            raise ValueError(
                f"ground_truth_scores ({len(y_true)}) and predicted_scores ({len(y_pred)}) length mismatch"
            )

        if not y_true:
            raise ValueError("No scores to evaluate")

        # Dataset targets may be 0–100; model outputs often 0–1
        if max(abs(x) for x in y_true) > 1.5:
            y_true = [x / 100.0 for x in y_true]
        if max(abs(x) for x in y_pred) > 1.5:
            y_pred = [x / 100.0 for x in y_pred]
        ...
        return {
            ...
            "test_samples": n,
            "successful_predictions": sum(1 for s in y_pred if s > 0),
            "failed_predictions": sum(1 for s in y_pred if s == 0),
            "regression_metrics": self._calculate_regression_metrics(y_true, y_pred),
            "ranking_metrics": self._calculate_ranking_metrics(y_true, y_pred),
            "classification_metrics": self._calculate_classification_metrics(y_true, y_pred),
            "tertile_bucket_metrics": self._calculate_tertile_bucket_metrics(y_true, y_pred),
            "performance_metrics": {
            ...
            "statistical_analysis": self._calculate_statistical_analysis(y_true, y_pred),
            "error_analysis": self._analyze_errors(y_true, y_pred, resumes, jobs),
        }

    def _calculate_tertile_bucket_metrics(
        self, y_true: List[float], y_pred: List[float]
    ) -> Dict[str, Any]:
        """0–100-style tertiles: [0,33), [33,66), [66,100] on scores scaled to 0–100."""

        def to_bucket(v: float) -> int:
            x = min(100.0, max(0.0, v * 100.0))
            if x < 33.0:
                return 0
            if x < 66.0:
                return 1
        ...
        yt = [to_bucket(x) for x in y_true]
        yp = [to_bucket(x) for x in y_pred]
        acc = accuracy_score(yt, yp)
        return {
            "tertile_classification_accuracy": float(acc),
