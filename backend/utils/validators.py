...
class RequestValidator:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.max_files = app_config.MAX_FILES_PER_REQUEST
        self.max_request_bytes = app_config.MAX_CONTENT_LENGTH_BYTES
        self.allowed_extensions = set(app_config.ALLOWED_EXTENSIONS)
        self._per_file_max_bytes = app_config.MAX_FILE_SIZE_MB * 1024 * 1024
        
        # Valid algorithms
        # xlm: not registered in AlgorithmManager (would load a heavy XLM model on 8GB setups).
        ...
        self.validation_rules = {
            'job_description': {
                'min_length': 20,
                'max_length': app_config.MAX_JD_LENGTH,
            ...
            'files': {
                'min_count': 1,
                'max_count': app_config.MAX_FILES_PER_REQUEST,
                'min_file_size': 100,  # 100 bytes
                'max_file_size': self._per_file_max_bytes,
                'allowed_mime_types': [
                    'application/pdf',
    ...
    def _validate_request_structure(self, request: Request) -> Dict[str, Any]:
        ...
        # Check request size
        if request.content_length and request.content_length > self.max_request_bytes:
            return {
                'valid': False,
                'error': 'Request too large',
                'details': f'Request size exceeds aggregate upload budget (~{app_config.MAX_FILE_SIZE_MB * app_config.MAX_FILES_PER_REQUEST}MB)',
                'max_size_mb': self.max_request_bytes // (1024 * 1024),
    ...
    def _validate_cross_field_constraints(self, files: List, methods: List[str], 
        ...
        complex_algorithms = {'bert', 'distilbert', 'sbert', 'neural_network'}
